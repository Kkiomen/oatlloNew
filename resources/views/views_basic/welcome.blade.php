...
<style>
        ...
        :root { --accent: #f43f5e; } /* rose-500 – domyślny akcent home (blog); karty kursów nadpisują --accent */
        ...
        <div class="mt-14 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            @foreach($courses as $course)
                @php
                    ...
                    // Akcent per-kurs (motyw okładki) dla karty: nazwa palety + hex (hover).
                ...
                @endphp
                <a href="{{ $course->getRoute() }}" style="--accent: {{ $accentHex }}" class="card-hover group flex flex-col overflow-hidden rounded-3xl border border-white/10 bg-neutral-900">
                    <div class="relative h-44 overflow-hidden bg-neutral-800">
                        ...
                        <span class="absolute left-4 top-4 rounded-full bg-{{ $accent }}-500 px-3 py-1 text-xs font-semibold text-white shadow-lg shadow-{{ $accent }}-500/30">FREE</span>
                    </div>
                    <div class="flex flex-1 flex-col p-6">
                        <h3 class="text-xl font-bold text-white group-hover:text-{{ $accent }}-300 transition-colors duration-200">{{ $course->title_list ?: $course->name }}</h3>
                        ...
                        <div class="mt-5 flex items-center justify-between">
                            ...
                            <span class="inline-flex items-center gap-2 text-sm font-semibold text-{{ $accent }}-400 group-hover:gap-3 transition-all duration-200">
