...
1. Which syntax defines a constant inside a class?
...
1. Which statement about `define()` in namespaces is correct?
...
1. Which of the following values can be assigned to a constant via `const`?
...
1. How to check if a constant named `APP_ENV` is defined?
...
1. Which operator is used to access a class constant?
...
1. True/False: `const` can be used inside a function to define a global constant.

- Answer: ...

1. Task: Write a one-liner using `constant()` to display the value of a constant stored in the variable `$name`.
