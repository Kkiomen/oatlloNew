...
1. Syntax for foreach with key and value? ➡️ `foreach ($arr as $key => $value) { ... }`
2. Why use `&` in foreach? What to do after? ➡️ Modify originals; unset($value) after.
3. foreach on null? ➡️ Warning; fix with `(array)$x` or `is_iterable($x)`.
4. Will added element be visited immediately? ➡️ Usually no.
5. How to destructure `[x,y]` arrays? ➡️ `foreach ($points as [$x, $y])`
6. What does foreach iterate on stdClass? ➡️ Public properties.
7. When use generators with foreach? ➡️ Large/streaming data.
8. Why is foreach better than for for arrays? ➡️ Clearer, avoids index/count issues.
