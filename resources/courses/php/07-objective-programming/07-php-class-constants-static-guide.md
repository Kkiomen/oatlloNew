...
1. Key difference between `self::` and `static::`? ➡️ `self::` binds to where code is written, `static::` uses late binding.
2. Can class constants be modified at runtime? ➡️ No.
3. How to access constant `ROLE_ADMIN` in `User` class externally? ➡️ `User::ROLE_ADMIN`.
...
1. When use `new static()` vs `new self()`? ➡️ `new static()` supports polymorphism.
2. Example of typed constant in PHP 8.3? ➡️ `public const int COUNT = 10;`.
3. Since which PHP version can constants have visibility? ➡️ PHP 7.1.
4. What does `(new Y())->dbg();` print here?
