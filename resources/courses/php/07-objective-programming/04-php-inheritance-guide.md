...
1. Which keyword makes class A inherit from class B? ➡️ `extends`
2. What does `parent::__construct()` do in a child class? ➡️ Calls the parent’s constructor.
3. Which visibility is accessible in subclasses? ➡️ protected and public.
4. When to use `parent::method()`? ➡️ When overriding but also needing parent logic.
5. What does `final` mean for a method? ➡️ Cannot be overridden.
6. Can you use `parent::` outside a class? ➡️ No.
