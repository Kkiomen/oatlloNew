...
- Checking values without confirming key existence:  Instead:
...
1. Which syntax is recommended in PHP? ➡️ `elseif`
2. What’s the output?
...
1. Safer way to check user’s age in `$u`: ➡️ `if (isset($u["age"]) && $u["age"] >= 18) { ... }`
2. Difference between `&&` and `and`? ➡️ Both are logical AND, but with different precedence.
3. What prints?
...
1. In an `if ... elseif ... else` chain, which block executes? ➡️ Only the first block where the condition is true.
2. True or false: `empty([])` returns true. ➡️ True
3. What’s the result?
...
1. Which comparison is safer for form data (string vs int)? ➡️ `===`
