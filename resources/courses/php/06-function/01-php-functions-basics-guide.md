...
1. What does return do in PHP functions? ➡️ Ends function and optionally returns a value.
2. Output?
...
1. Correct definition? ➡️ `function sum(int $a, int $b): int { return $a + $b; }`
2. Difference echo vs return? ➡️ return gives a value; echo prints.
3. Which definition of optional parameter is correct? ➡️ `function x($a, $b = 10) { ... }`
4. ?string means? ➡️ string or null.
5. Output of g(2,3,4)? ➡️ 9
6. Function with void return type? ➡️ cannot return a value.
7. Code after return runs? ➡️ False.
8. never type means? ➡️ function never returns (throws or exits).
