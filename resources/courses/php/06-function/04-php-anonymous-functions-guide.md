...
1. What does `use` do in anonymous functions? ➡️ Captures variables from the outer scope.
2. Difference between `callable` and `Closure`? ➡️ `Closure` is a specific object, `callable` is any callable (string, array, Closure).
3. Output?
...
1. Which statement about arrow functions is true? ➡️ They auto-capture variables by value and are one-liners.
2. Fill in to sort descending:
...
1. How to create a `Closure` from `trim` in PHP 8.1+? ➡️ `$trim = trim(...);`
2. One-line arrow function to return last character of string? ➡️ `$lastChar = fn(string $s): string => substr($s, -1);`
