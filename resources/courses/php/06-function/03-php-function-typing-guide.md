...
1. What does `function f(?int $x): void` mean? ➡️ $x may be int or null; function returns nothing.
2. With strict_types=1, what happens when calling `add("2", "3")`? ➡️ Throws `TypeError`.
3. Can a void function return a value? ➡️ No.
4. When to use mixed? ➡️ Only if multiple unknown types possible.
5. How to define an optional int parameter that may be null? ➡️ `function f(?int $x = null) {}`
6. What does object type mean? ➡️ Any instance of any class.
7. What will `isAdult(18)` return? ➡️ true.
8. What does `?int` as return type mean? ➡️ Function may return int or null.
