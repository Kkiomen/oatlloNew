...
1. Which function checks if a key exists even if its value is null?

- a) isset($arr['key'])
- b) in_array('key',$arr)
- c) array_key_exists('key',$arr)

1. Which function sorts an associative array by values while preserving keys?

- a) sort
- b) asort
- c) ksort

1. How to find index of 'Ola' strictly in ['Ada','Ola','Jan']?

- a) array_search('Ola',$arr)
- b) array_search('Ola',$arr,true)
- c) in_array('Ola',$arr)

1. How to remove index 2 and shift others to avoid holes?

- a) unset($arr[2])
- b) array_splice($arr,2,1)
- c) array_shift($arr)

1. Difference between array_merge and + with associative arrays?

- a) Same
- b) array_merge overwrites with later values, + keeps left
- c) Opposite

1. Which function transforms each element?

- a) array_filter
- b) array_map
- c) array_reduce

1. How to sort users by 'age' ascending?
