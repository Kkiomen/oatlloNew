...
1. Which is true about array_walk?

- A: Returns new array.
- B: Modifies in place via reference.
- C: Works only on indexed arrays. ➡️ B

1. What does third param `true` in array_chunk($arr,2,true) do? ➡️ Preserves original keys in chunks.
2. Which to use for batch processing 100 elements? ➡️ array_chunk
3. Common mistake with foreach by reference? ➡️ Not unsetting reference after loop.
